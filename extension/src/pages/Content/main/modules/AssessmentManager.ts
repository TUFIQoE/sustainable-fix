...
export class AssessmentManager{
    ...
    private schedule_assessment_panel = () : void => {
        if(this.assessment_interval){
            this.logger.log(`Scheduling assessment in ${this.assessment_interval/1000}.`)
            setTimeout(() => {
     ...
     */
    create_buttons(){
        const descriptions = ["Not Annoying", "Annoying but Acceptable", "Not Acceptable"]
        const buttons : HTMLButtonElement[] = []
        descriptions.forEach((text, index) => {
            const button = document.createElement("button")
            const value = descriptions.length - index
